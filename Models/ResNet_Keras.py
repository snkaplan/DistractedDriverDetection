...
def createModel():
    base_model=ResNet50(weights=None,include_top=False,input_shape=(img_width,img_height,color_type))
    x=base_model.output
...
checkpoint = ModelCheckpoint('../HistoryAndWeightFiles/ResNet_Keras_model_weights_v1.h5', monitor='val_loss', verbose=1, save_best_only=True, mode='min')
...
with open("../HistoryAndWeightFiles/ResNet_Keras_Model_History_v2.json","w") as f:  ##modelin accuracy değerlerini jsona yazar
    json.dump(histt,f) 
#%%Model Load
#model.load_weights('../HistoryAndWeightFiles/Indception_Model_Weights_v2.h5') #kayıtlı değişkenleri modele yükler

with codecs.open("../HistoryAndWeightFiles/ResNet_Keras_Model_History_v2.json","r",encoding = "utf-8") as f: #accuracy ve lost değişkenlerini tekrar yükler
